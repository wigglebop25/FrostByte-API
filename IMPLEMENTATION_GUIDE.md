...
This guide is your daily "instruction manual" for building the FrostByte API. It breaks down the `MASTER_PLAN.md` into specific coding tasks.
...
*Action:* Write the `Config` struct and `LoadConfig()` function to read environment variables (see `MASTER_PLAN.md` Appendix B).
...
*Action:* Define a MySQL service (port 3306) and the Go app service (see `MASTER_PLAN.md` Phase 1).

### 2. Domain Models
**File:** `internal/domain/models.go`
*Action:* Copy the structs (`User`, `Product`, `Order`, etc.) from **Appendix C** of the Master Plan.
...
*Action:* Copy the `Hub` code from **Appendix D** of the Master Plan.
